...
class ImageClassifier:
    ...
    def predict_batch(self,images : Union[str,bytes]):
        ...
        predicted_class_indices = np.array(predictions>0.5).astype(int).ravel()
        predicted_class_names = [self.idx2label[i] for i in predicted_class_indices]
        confidence_score = np.where(predicted_class_indices==1,
                                    predictions.ravel()*100,
                                    (1-predictions.ravel()*100))
